...
一个从零搭建的 Unreal 动作游戏 Demo。角色是 Paragon 的 Crunch——一个近战拳击手，核心战斗围绕**轻击连招、浮空 UpperCut、地面 AOE 轰炸**展开。底层完全基于 **Gameplay Ability System** 搭建，"服务器权威"模型做网络同步。
...
选 GAS 的核心原因：**网络同步是"内置"的，不是"后加"的**。

做技能系统时，如果你的 Actor 需要 `HasAuthority()` → 做伤害 → 再手动 `Client_RPC` 去播特效，这个模式写三五个技能还行，写到第十个的时候一定会出问题——要么忘了权限判断导致作弊，要么忘了客户端回调导致特效不播。
...
**双层 AttributeSet 设计**是这个项目的一个关键架构决策：
...
这是整个项目最关键的网络架构模式。看一下 `CCharacter` 的初始化流程：
...
> **关键设计原则：服务器初始化"数据"，客户端只初始化"连接"**。服务器拥有权威数据（属性、技能列表、效果），客户端通过 GAS 内置的属性复制机制自动同步，不需要手动 RPC 把属性一个个发过去。
...
UpperCut 不仅是一个独立技能，它还能**接入 Combo 系统**——在 Combo 的任意段位之后接 UpperCut，伤害系数随 Combo 段位变化：
...
> **教训：GAS 的默认设置不一定适合你的场景**。GameplayCue 默认走 RPC 是为了保证服务器也能"看见" Cue 做后续逻辑，但如果你的 Cue 纯视觉，local 就足够了。
...
正确做法：在 `PostGameplayEffectExecute` 里做逻辑响应（死亡判定、UI 更新）。
...
复活时倒序执行，并且**只在服务器**移除死亡 GE、重置位置到 StartSpot。

---

## 五、架构可改进点

写给自己看的反思，也是给面试官展示"我知道哪里还不够好"：
...
Crunch 是一个"做给自己看"的 Demo——验证 GAS 架构思路，踩一遍网络同步的坑。下一个项目计划侧重：
