...
这是一个**为 Vibe Coding 设计的 UE5 开发框架**——不是传统意义上的游戏项目，而是一套让 AI 能可靠地参与 UE5 游戏开发的工程基础设施。它回答了"当 AI 写游戏代码时，怎么保证架构正确、编译通过、资产就位、行为验收？"这个问题。
...
这是整个框架最重要的设计决策。借鉴软件工程中的需求层级，我将游戏开发需求也拆为三层：
...
并且支持**多对多关系**：一个 Concept 可以产出多个 Order，多个 Concept 可以共享一个 Order。进度沿全链路向上聚合。

### 2.3 最小充分链路原则

并非所有任务都要走 Concept → Order → REQ → 实现 → 归档的全链。协调者根据用户意图做**路由判断**：
...
这个分级的核心思想是：**验证不允许降级，自动化允许回退**。编译门和资产完整性检查是质量的最后防线，不能因为"编辑器没启动"就跳过。
...
我设计了两份**性质不同**的清单，由不同 agent 负责产出和验证：
...
**关键洞察**：把"让功能存在"和"让功能正确"分开，避免了"编译通过了但 PIE 进不去"的死循环。

---

## 五、核心设计三：流程 Bug 反馈闭环

这是框架中最独特的机制——**Bug 不仅是代码 bug，也包括流程 bug、架构决策 bug**。
...
Lyra 核心类没有 `LYRAGAME_API` 导出宏，意味着**不能从外部 GameFeature 插件的 C++ 代码中继承它**。正确做法是用蓝图 DataAsset 实例化，而非 C++ 子类化。
...
最有价值的教训：**代码审查的 15 个维度全过了，但架构选型是错的**。430 行 `ATowerCharacter` 通过了所有审查标准——代码风格、GC 安全、网络同步、GAS 模式——但本质上是错的，因为在 Lyra 体系中，创建 Character C++ 子类是反模式。这暴露了"审查维度完备 ≠ 审查结论正确"，需要在维度之上加一层架构级硬约束。
